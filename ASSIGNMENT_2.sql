...
select title as movie_title, c.first_name, c.last_name from film f join inventory i on f.film_id = i.film_id join rental r on i.inventory_id = r.inventory_id join customer c on r.customer_id = c.customer_id;
...
WHERE
    title = 'Gone with the Wind'; --  I couldn't find any film title as 'Gone with the wing'
...
SELECT
    name AS language, AVG(rental_rate) AS avg_rental_rate
FROM
    film f
        JOIN
...
SELECT 
    c.first_name,
    c.last_name,
    SUM(p.amount) AS total_amount_sepnt
FROM
    customer c
        JOIN
